...
function withConfigCatClient<P>(WrappedComponent: React.ComponentType<P & WithConfigCatClientProps>): React.ComponentType<Omit<P, keyof WithConfigCatClientProps>> {
    return (props: P) => (
            <ConfigCatContext.Consumer>
                {context => {
                    if (context === undefined) {
                        throw new Error('ConfigCatContext.Consumer must be used within a ConfigCatProvider')
                    }
                    return <WrappedComponent configCatClient={context.client} getValue={getValueFunction(context.client)} {...(props as P)}/>
                }}
            </ConfigCatContext.Consumer>
          );
}
